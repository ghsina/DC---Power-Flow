    ...
    for i in range(1, n+1):
        if i == ref_bus:
            ...
            summary_data.append({
                'Node': f"Node {i}",
                'Type': 'PQ Bus',
                'Power': f"{st.session_state.powers[f'P_{i}']:.2f} MW",
...
# Step 5: Results and Calculations (MODIFIED SOLUTION)
    ...
    try:
        ...
        with col2:
            ...
            st.dataframe(df_P_known.style.format({'Power [MW]': '{:.4f}'}), use_container_width=True)
        ...
        st.success(f"""
        **P_{ref_bus} = {P_ref:.4f} MW**
        
        This is calculated using: P_{ref_bus} = Σ B_{{{ref_bus},j}} × θ_j
        
        The reference bus power balances the system:
        - Total Power Specified: {np.sum([P_complete[i] for i in range(n) if i != ref_idx]):.4f} MW
        - Reference Bus Power: {P_ref:.4f} MW
        - Total System Power: {np.sum(P_complete):.4f} MW (should be ≈ 0 for balanced system)
        """)
        ...
        st.dataframe(power_df.style.format({'Power [MW]': '{:.4f}'}), use_container_width=True)
        ...
        with col1:
            st.metric("Total Generation", f"{total_gen:.2f} MW")
        with col2:
            st.metric("Total Load", f"{total_load:.2f} MW")
        with col3:
            st.metric("Power Balance Error", f"{power_balance:.6f} MW", 
        ...
        **Verification Steps:**
        ...
        export_data += f"Reference Bus Power (Calculated): {P_ref:.6f} MW\n\n"
        ...
        export_data += f"\n\nPower Balance: {np.sum(P_complete):.10f} MW\n"
